...
students.Add(new Student("Kelly", 6.5));
students.Add(new Student("Teena", 8.3));
students.Add(new Student("Jessika", 8.7));
students.Add(new Student("Lola", 6.4));
students.Add(new Student("Greta", 7.0));
students.Add(new Student("Vic", 6.7));

students.ForEach(s =>
{
    if (s.Grade > 6.5 && s.Grade < 7.0)
    {
        s.Grade += 0.5;
    }
});
...
students.ForEach(s => Console.WriteLine($"Nome: {s.Name} Nota: {s.Grade}"));
